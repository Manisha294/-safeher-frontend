    ...
    try {
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `You are a women safety AI assistant.
A woman wants to travel to: "${destination}" in Hyderabad, India.
Current time: ${new Date().toLocaleTimeString()}

Give her:
1. Safety tips for this route
2. Recommended safe travel mode (auto/cab/bus/walk)
3. What to watch out for
4. Emergency numbers to keep ready
...
Keep it short, friendly and practical. Use emojis.`
      ...
      const text = data.content?.[0]?.text || 'Could not get route advice. Try again.'
      setAiRouteResult(text)
    } catch {
      setAiRouteResult('⚠ AI service unavailable. Please try again.')
    }
    setAiLoading(false)
  }
